...
#include "nn/evaluation/classification.h"

namespace fs = std::filesystem;

#ifndef MLPP_PROJECT_ROOT
#define MLPP_PROJECT_ROOT "."
#endif

int main(int argc, char** argv) {
    try {
        fs::path dataDir = fs::path(MLPP_PROJECT_ROOT) / "data" / "mnist";

        if (argc >= 2) {
            dataDir = fs::path(argv[1]);
        }

        const std::string trainImagesPath = dataDir / "train-images-idx3-ubyte";
        const std::string trainLabelsPath = dataDir / "train-labels-idx1-ubyte";
        const std::string testImagesPath  = dataDir / "t10k-images-idx3-ubyte";
        const std::string testLabelsPath  = dataDir / "t10k-labels-idx1-ubyte";

        std::cout << "MNIST data dir: " << dataDir << "\n";
